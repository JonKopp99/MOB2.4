...
final class ExtraCheeseTopping: ToppingsDecorator {
    
    //TODO: Implement Extra Cheese -- add 1.0 to current price
    override func getPrice() -> Double {
        let pizzaPrice = super.getPrice()
        return pizzaPrice + 1.0
    }
}

// Decorator class (extended)
final class MushroomTopping: ToppingsDecorator {
    
    //TODO: Implement adding mushrooms -- add 1.49 to current price
    override func getPrice() -> Double {
        let pizzaPrice = super.getPrice()
        return pizzaPrice + 1.49
    }
    
}

// Decorator class (extended)
final class JalapenoTopping: ToppingsDecorator {
    
    //TODO: Implement JalapenoToppingk, add an extra 1.19 for peppers
    override func getPrice() -> Double {
        let pizzaPrice = super.getPrice()
        return pizzaPrice + 1.19
    }
}
